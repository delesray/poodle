...
async def test_create_user_raises_409(db):
    ...
    mock_user.password = account.password
...
@pytest.mark.asyncio
async def test_get_create_user_happy_case(db):
    test_email = 'test@email.com'
    test_pass = 'testpass'
    account, _ = await dummies.create_dummy_admin(db)
    mock_user = Mock()
    mock_user.email = test_email
    mock_user.password = test_pass
    mock_user.get_type = lambda: 'admin'
