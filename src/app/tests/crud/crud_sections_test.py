...
@pytest.mark.asyncio
async def test_add_student_if_student_viewed_section(db, test_db, mocker):
...
def test_validate_section_1(db, test_db):
...
@pytest.mark.asyncio
async def test_validate_section_happy(db, test_db):
    section, course = await dummies.create_dummy_section(db)
